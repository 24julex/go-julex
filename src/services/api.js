...
export const api = {
  ...
  orders: {
    ...
    collectCod: (id) => request(`/orders/${encodeURIComponent(id)}/collect-cod`, { method: 'PATCH' }),
    delete: (id) =>
      request(`/orders/${id}`, {
        method: 'DELETE'
      })
  ...
  // Coupons endpoints
  coupons: {
    getActive: (subdomain) => request(`/coupons${subdomain ? `?subdomain=${encodeURIComponent(subdomain)}` : ''}`),
    update: (code, patch) =>
