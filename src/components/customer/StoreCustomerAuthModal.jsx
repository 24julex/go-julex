...
export const StoreCustomerAuthModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in text-[#0F172A]">
      ...
      <div className="relative w-full max-w-md bg-white border border-[#E7D9B5] rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5 z-10">
        {/* Close Button */}
        <button
          type="button"
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-xl bg-[#FFFDF5] hover:bg-slate-100 text-[#475569] hover:text-[#0F172A] border border-[#E7D9B5] transition cursor-pointer"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="text-center space-y-1">
          <span className="text-[10px] uppercase tracking-widest text-[#6B4D00] font-bold block bg-[#FFFDF5] px-3 py-1 rounded-full border border-[#DCC78B] w-fit mx-auto">
        ...
        {!customerUser && (
          <div className="grid grid-cols-2 p-1 rounded-2xl bg-[#FFFDF5] border border-[#DCC78B] text-xs font-bold">
            <button
              type="button"
              onClick={() => { setAuthMode('signin'); setError(''); setSuccessMsg(''); }}
              className={'py-2 rounded-xl transition cursor-pointer ' + (authMode === 'signin' ? 'bg-white text-[#6B4D00] shadow-xs' : 'text-[#475569] hover:text-[#0F172A]')}
            >
              Sign In
            </button>
            <button
              ...
              className={'py-2 rounded-xl transition cursor-pointer ' + (authMode === 'signup' ? 'bg-white text-[#6B4D00] shadow-xs' : 'text-[#475569] hover:text-[#0F172A]')}
            >
              New Customer
            </button>
          </div>
        ...
        {customerUser ? (
          <div className="space-y-4 text-left">
            <div className="p-4 rounded-2xl bg-[#FFFDF5] border border-[#E7D9B5] flex items-center justify-between text-xs">
              <div>
                <p className="font-bold text-[#0F172A] flex items-center gap-1.5">
                  <User className="w-3.5 h-3.5 text-[#8A6200]" /> {customerUser.name}
                </p>
                <p className="text-[#475569] text-[11px] font-mono">{customerUser.email}</p>
              </div>
              <button
                onClick={handleSignOut}
                className="px-3 py-1.5 rounded-xl bg-white hover:bg-rose-50 text-rose-700 border border-[#E7D9B5] font-bold text-[11px] flex items-center gap-1.5 transition cursor-pointer"
              >
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between text-xs font-bold text-[#0F172A]">
                <span className="flex items-center gap-1.5">
                  <ShoppingBag className="w-4 h-4 text-[#8A6200]" /> Your Order History ({customerOrders.length})
                </span>
              ...
              {customerOrders.length > 2 && (
                <input
                  ...
                  className="w-full px-3 py-1.5 rounded-xl border border-[#E7D9B5] bg-white text-xs text-[#0F172A] focus:outline-none focus:border-[#8A6200]"
                />
              )}

              {customerOrders.length === 0 ? (
                <div className="p-6 rounded-2xl bg-stone-50 border border-[#E7D9B5] text-center space-y-2">
                  <Clock className="w-8 h-8 mx-auto text-slate-300" />
                  <p className="text-xs font-semibold text-[#475569]">No orders placed yet at this store.</p>
                  <button
                    onClick={onClose}
                    className="px-4 py-2 rounded-xl bg-[#8A6200] text-white font-bold text-xs shadow-xs cursor-pointer"
                  >
                    Start Shopping
                  </button>
                </div>
                ...
                <div className="space-y-2.5 max-h-72 overflow-y-auto pr-1">
                  {customerOrders.map((ord) => {
                    ...
                    return (
                      <div
                        key={ord.id}
                        className="p-3.5 rounded-2xl border border-[#E7D9B5] bg-white text-xs space-y-2 shadow-xs transition hover:border-[#8A6200]/40"
                      >
                        {/* Order Header Summary */}
                        <div className="flex items-center justify-between">
                          <div>
                            <span className="font-mono font-bold text-[#8A6200] text-sm block">
                              {ord.id}
                            </span>
                        ...
                        <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                          <button
                            ...
                            className="text-[11px] font-bold text-[#8A6200] hover:underline flex items-center gap-1 cursor-pointer"
                          >
            ...
            <button
              onClick={onClose}
              className="w-full py-3 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-md transition cursor-pointer"
            >
          ...
          <form onSubmit={handleSignIn} className="space-y-3.5 text-left">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-3.5 py-2.5 bg-white border border-[#E7D9B5] rounded-xl text-[#0F172A] text-xs placeholder:text-slate-400 focus:outline-none focus:border-[#8A6200] focus:ring-2 focus:ring-rose-100"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-lg shadow-rose-900/20 transition transform active:scale-98 flex items-center justify-center gap-2 disabled:opacity-60 cursor-pointer"
            >
              {loading ? (
                <>
