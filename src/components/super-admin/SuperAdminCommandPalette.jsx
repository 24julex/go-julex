...
export const SuperAdminCommandPalette = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-16 sm:pt-24 px-4 bg-[#0F172A]/60 backdrop-blur-sm animate-fade-in text-[#0F172A]">
      ...
      {/* Palette Container */}
      <div className="relative w-full max-w-2xl bg-white border border-[#E7D9B5] rounded-3xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[80vh]">
        {/* Search Input Box */}
        <div className="flex items-center px-4 py-3.5 border-b border-[#E7D9B5] bg-white">
          <Search className="w-5 h-5 text-[#8A6200] shrink-0 mr-3" />
          <input
            ref={inputRef}
            type="text"
            value={query}
          ...
          {query && (
            <button
              onClick={() => setQuery('')}
              className="p-1 rounded-xl hover:bg-[#FFFDF5] text-slate-400 hover:text-slate-600 text-xs mr-2"
            >
              Clear
            </button>
          )}
        ...
        {/* Results Body */}
        <div className="overflow-y-auto p-3 space-y-4 text-xs divide-y divide-[#E7D9B5]/60">
          {/* Matched Tenants */}
          {filteredTenants.length > 0 && (
            <div className="space-y-1.5 pt-1">
              <div className="text-[10px] font-bold uppercase tracking-wider text-[#6B4D00] px-2 flex items-center justify-between">
              ...
              <div className="space-y-1">
                {filteredTenants.map((tenant) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2.5 rounded-2xl hover:bg-[#FFFDF5] cursor-pointer group transition border border-transparent hover:border-[#E7D9B5]"
                  >
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        alt={tenant.name}
                        className="w-8 h-8 rounded-xl object-cover border border-[#E7D9B5] shrink-0"
                      />
                      <div>
                        <div className="font-semibold text-[#0F172A] group-hover:text-[#8A6200] flex items-center gap-2">
                          {tenant.name}
                          <span
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-xl bg-[#FFFDF5] hover:bg-[#F8E8BD] border border-[#DCC78B] text-[#6B4D00] text-[11px] font-medium transition flex items-center gap-1"
                        title="Impersonate (View as Merchant)"
                      >
                        <UserCheck className="w-3 h-3 text-[#8A6200]" /> Impersonate
                      </button>
                      <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-[#8A6200] group-hover:translate-x-0.5 transition" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {filteredActions.length > 0 && (
            <div className="space-y-1.5 pt-3">
              <div className="text-[10px] font-bold uppercase tracking-wider text-[#6B4D00] px-2">
                Quick Actions
              </div>
                ...
                {filteredActions.map((act, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center gap-2.5 p-2 rounded-2xl bg-white hover:bg-[#FFF3CE] cursor-pointer group transition border border-[#E7D9B5]"
                    >
                      <div className="w-7 h-7 rounded-xl bg-[#FFFDF5] border border-[#DCC78B] flex items-center justify-center text-[#8A6200] shrink-0">
              ...
              {filteredNav.map((mod) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center justify-between p-2 rounded-2xl hover:bg-[#FFFDF5] cursor-pointer group transition border border-transparent hover:border-[#E7D9B5]"
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="w-6 h-6 rounded-lg bg-[#FFFDF5] flex items-center justify-center text-[#6B4D00] group-hover:text-[#0F172A] transition">
                        <Icon className="w-3.5 h-3.5" />
                      </div>
                    ...
                    <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#8A6200]" />
                  </div>
                );
              })}
            </div>
        ...
        {/* Footer shortcuts */}
        <div className="px-4 py-2.5 bg-white border-t border-[#E7D9B5] flex items-center justify-between text-[11px] text-[#374151]">
          <div className="flex items-center gap-3">
            <span>
              <kbd className="px-1.5 py-0.5 rounded bg-white text-[10px] text-[#6B4D00] border border-[#E7D9B5]">
                ↑
              </kbd>{' '}
              <kbd className="px-1.5 py-0.5 rounded bg-white text-[10px] text-[#6B4D00] border border-[#E7D9B5]">
                ↓
          ...
          <span className="text-[#8A6200] font-mono text-[10px]">Go Julex Master v2.4</span>
        </div>
      </div>
    </div>
  );
