...
export const TenantDetailDrawer = ({ tenantId, isOpen, onClose }) => {
  ...
  const getSegmentColor = (segment) => {
    switch (segment) {
      ...
      case 'Purchased once':
        return 'bg-[#FFFDF5] text-[#6B4D00] border border-[#DCC78B]';
      case 'Just a viewer':
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden animate-fade-in text-[#0F172A]">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-2xl bg-white border-l border-[#E7D9B5] shadow-2xl flex flex-col">
          {/* Drawer Header */}
          <div className="px-6 py-5 border-b border-[#E7D9B5] bg-white flex items-center justify-between">
            <div className="flex items-center gap-3.5">
              <img
                ...
                alt={tenant.name}
                className="w-12 h-12 rounded-2xl object-cover border border-[#E7D9B5] shadow-xs"
              />
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-[#374151] mt-1">
                  <span className="font-mono text-[11px] text-[#8A6200]">{tenant.id}</span>
                  <span>•</span>
                  <span>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-[#FFFDF5] hover:bg-[#F8E8BD] border border-[#DCC78B] text-[#6B4D00] text-xs font-bold transition flex items-center gap-1.5 shadow-xs"
                title="View as Merchant"
              >
                <UserCheck className="w-3.5 h-3.5 text-[#8A6200]" /> View as Merchant
              </button>
              <button
                onClick={onClose}
                className="p-1.5 rounded-xl hover:bg-[#FFFDF5] text-slate-400 hover:text-slate-600 transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
          ...
          {/* Navigation Tabs */}
          <div className="flex border-b border-[#E7D9B5] bg-[#FFFDF5]/60 px-6 gap-6 text-xs font-semibold overflow-x-auto">
            <button
              onClick={() => setActiveTab('products')}
              className={`py-3 flex items-center gap-2 border-b-2 transition relative whitespace-nowrap ${
                activeTab === 'products'
                  ? 'border-[#A87A00] text-[#0F172A] font-bold'
                  : 'border-transparent text-[#374151] hover:text-[#0F172A]'
              }`}
            >
              <Package className="w-3.5 h-3.5 text-[#8A6200]" /> Live Store Catalog
              <span className="px-2 py-0.2 rounded-full bg-[#FFFDF5] text-[10px] font-bold text-[#6B4D00] border border-[#DCC78B]">
            ...
            >
              ...
              <span className="px-1.5 py-0.2 rounded-full bg-[#FFFDF5] text-[10px] text-[#6B4D00] border border-[#DCC78B]">
          ...
          <div className="flex-1 overflow-y-auto p-6 text-xs">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between border-b border-[#E7D9B5] pb-3">
                  <div>
                    <h4 className="font-bold text-[#0F172A] text-xs uppercase tracking-wider flex items-center gap-2">
                      <Package className="w-3.5 h-3.5 text-[#8A6200]" /> Live Merchant Catalog ({tenantProducts.length} Items)
                    </h4>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-[10px] font-bold shadow-xs transition"
                  >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-2 bg-white border border-[#E7D9B5] rounded-2xl text-[11px] text-[#0F172A] focus:outline-none focus:border-[#A87A00]"
                  />
                </div>

                {/* Products Table */}
                {tenantProducts.length === 0 ? (
                  <div className="p-8 rounded-3xl bg-[#FFFDF5]/30 border border-[#E7D9B5] text-center space-y-2">
                    <Package className="w-8 h-8 mx-auto text-[#8A6200] opacity-60" />
                    ...
                    {tenantProducts
                      ...
                      .map((prod, idx) => {
                        ...
                        return (
                          <div
                            key={prod.id || idx}
                            className="p-3 rounded-2xl bg-white border border-[#E7D9B5] hover:border-[#A87A00]/50 transition flex items-center justify-between gap-3 shadow-2xs"
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <img
                                src={img}
                                alt={prod.name}
                                className="w-12 h-12 rounded-xl object-cover border border-[#E7D9B5] shrink-0 bg-stone-50"
                              />
                              <div className="min-w-0">
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-[#374151] mt-0.5">
                                  <span className="font-semibold text-[#6B4D00]">{prod.category || tenant.category}</span>
                                  <span>•</span>
                            ...
                            <div className="flex items-center gap-3 shrink-0 text-right">
                              <div>
                                <div className="font-mono font-bold text-xs text-[#8A6200]">
            ...
            {activeTab === 'config' && (
              <form onSubmit={handleSave} className="space-y-6">
                ...
                <div className="space-y-4">
                  <h4 className="font-bold text-[#0F172A] text-xs uppercase tracking-wider flex items-center gap-2">
                    <Store className="w-3.5 h-3.5 text-[#8A6200]" /> Store Identifiers
                  </h4>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        disabled
                        className="w-full px-3 py-2 bg-white/50 border border-[#E7D9B5] rounded-2xl text-slate-400 cursor-not-allowed font-mono text-[11px]"
                      />
                    </div>
                  </div>
                </div>
                ...
                {/* Plan & Subscription */}
                <div className="space-y-4 pt-4 border-t border-[#E7D9B5]">
                  <h4 className="font-bold text-[#0F172A] text-xs uppercase tracking-wider flex items-center gap-2">
                    <CreditCard className="w-3.5 h-3.5 text-[#8A6200]" /> Plan & Status
                  </h4>
                ...
                <div className="space-y-4 pt-4 border-t border-[#E7D9B5]">
                  <h4 className="font-bold text-[#0F172A] text-xs uppercase tracking-wider flex items-center gap-2">
                    <User className="w-3.5 h-3.5 text-[#8A6200]" /> Primary Merchant Admin
                  </h4>
                ...
                <div className="space-y-2 pt-4 border-t border-[#E7D9B5]">
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-white border border-[#E7D9B5] rounded-2xl text-[#0F172A] focus:outline-none focus:border-[#A87A00] text-xs"
                    placeholder="Add operational notes or compliance records..."
                  />
                </div>

                {/* Actions */}
                <div className="flex items-center justify-between pt-4 border-t border-[#E7D9B5]">
                  <button
                    type="button"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold flex items-center gap-1.5 shadow-xs transition"
                    >
            ...
            {activeTab === 'customers' && (
              <div className="space-y-4">
                <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                  <div>
                    <h4 className="font-bold text-[#0F172A] text-xs flex items-center gap-2 font-serif">
                      <Users className="w-4 h-4 text-[#8A6200]" />
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <select
                      ...
                      className="px-3 py-1.5 bg-white border border-[#E7D9B5] rounded-xl text-[11px] text-[#0F172A] focus:outline-none focus:border-[#A87A00]"
                    >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-white border border-[#E7D9B5] rounded-2xl text-[#0F172A] placeholder-stone-400 text-xs focus:outline-none focus:border-[#A87A00]"
                  />
                </div>
                ...
                <div className="space-y-2">
                  {filteredCustomers.length === 0 ? (
                    <div className="text-center py-10 border border-dashed border-[#E7D9B5] rounded-3xl">
                      <Users className="w-8 h-8 text-[#8A6200] mx-auto mb-2 opacity-50" />
                    ...
                    filteredCustomers.map((customer) => (
                      <div
                        key={customer.id}
                        className="p-3.5 rounded-2xl bg-white border border-[#E7D9B5] hover:border-[#A87A00] transition flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs"
                      >
                        ...
                        <div className="flex items-center gap-4 text-right shrink-0">
                          <div>
                            <div className="text-xs font-bold text-[#8A6200] font-mono">
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] shadow-xs">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#6B4D00]">Store GMV</span>
                    <div className="text-lg font-bold text-[#8A6200] font-mono mt-1">
                  ...
                  <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] shadow-xs">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#6B4D00]">Total Orders</span>
                ...
                <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] space-y-3 shadow-xs">
