...
export const ThemeSandboxModal = ({ theme, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0F172A]/60 backdrop-blur-sm animate-fade-in text-[#0F172A]">
      ...
      <div className="relative w-full max-w-6xl h-[92vh] bg-white border border-[#E7D9B5] rounded-3xl shadow-2xl overflow-hidden z-10 flex flex-col">
        {/* Sandbox Header */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-3.5 border-b border-[#E7D9B5] bg-white gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <span className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse" />
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-sm text-[#0F172A] font-serif">{theme.name}</h3>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#FFFDF5] text-[#6B4D00] border border-[#DCC78B]">
          ...
          {/* Viewport switcher */}
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-white p-1 rounded-2xl border border-[#E7D9B5]">
              <button
                onClick={() => setViewport('desktop')}
                className={`p-1.5 rounded-xl transition ${
                  viewport === 'desktop' ? 'bg-[#8A6200] text-white shadow-xs' : 'text-[#6B4D00]'
                }`}
                title="Desktop View"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-xl transition ${
                  viewport === 'tablet' ? 'bg-[#8A6200] text-white shadow-xs' : 'text-[#6B4D00]'
                }`}
                title="Tablet View"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-xl transition ${
                  viewport === 'mobile' ? 'bg-[#8A6200] text-white shadow-xs' : 'text-[#6B4D00]'
                }`}
                title="Mobile View"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-white hover:bg-[#FFF3CE] border border-[#E7D9B5] text-xs font-bold text-[#6B4D00]"
            >
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-2xl hover:bg-[#FFFDF5] text-slate-400 hover:text-slate-600 transition"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-hidden relative flex items-center justify-center p-4 bg-white">
          ...
          <div
            className={`h-full bg-white rounded-2xl shadow-xl border border-[#E7D9B5] overflow-y-auto transition-all duration-300 flex flex-col ${
              viewport === 'desktop'
                ? 'w-full'
                : viewport === 'tablet'
                ? 'w-[768px]'
          ...
          >
            ...
            <div
              ...
              style={{
                backgroundColor: theme.tokens?.primaryAccent || '#A87A00',
                color: '#FFFFFF'
              }}
            >
            ...
            <div className="py-4 px-6 border-b border-[#E7D9B5]/60 flex items-center justify-between">
              <h2
            ...
            <div className="relative aspect-[21/9] overflow-hidden bg-stone-100 flex items-center justify-center">
              ...
              <div className="absolute inset-0 bg-[#0F172A]/30 flex flex-col items-center justify-center text-center p-6 text-white space-y-2">
                ...
                <button
                  ...
                  style={{ backgroundColor: theme.tokens?.primaryAccent || '#A87A00' }}
                >
                  Explore Collection
                </button>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {currentMock.products.map((prod, idx) => (
                  <div
                    key={idx}
                    className={`p-3 border border-[#E7D9B5] shadow-xs flex flex-col justify-between ${
                      theme.tokens?.buttonRadius || 'rounded-2xl'
                    }`}
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <p
                        ...
                        style={{ color: theme.tokens?.primaryAccent || '#A87A00' }}
                      >
                        {prod.price}
                      </p>
                    </div>
          ...
          {/* JSON Schema Drawer */}
          {showJsonDrawer && (
            <div className="absolute top-0 right-0 bottom-0 w-96 bg-white border-l border-[#E7D9B5] shadow-2xl p-4 flex flex-col z-20 animate-slide-left text-xs">
              <div className="flex items-center justify-between pb-3 border-b border-[#E7D9B5]">
                <div className="flex items-center gap-2 text-[#0F172A] font-bold">
                  <FileCode className="w-4 h-4 text-[#8A6200]" /> Theme Token Schema JSON
                </div>
                <button
                  onClick={handleCopyJson}
                  className="px-2.5 py-1 rounded-xl bg-white hover:bg-[#FFF3CE] text-[#6B4D00] border border-[#E7D9B5] text-[11px] font-bold transition flex items-center gap-1"
                >
              ...
              <pre className="flex-1 overflow-y-auto p-3 mt-3 bg-white border border-[#E7D9B5] rounded-2xl font-mono text-[10px] text-[#0F172A] leading-relaxed select-all">
