...
export const CreateThemeModal = ({ isOpen, onClose, onSaveTheme }) => {
  ...
  const [primaryAccent, setPrimaryAccent] = useState('#A87A00');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0F172A]/60 backdrop-blur-sm animate-fade-in text-[#0F172A]">
      ...
      <div className="relative w-full max-w-2xl bg-white border border-[#E7D9B5] rounded-3xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E7D9B5] bg-white">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl bg-[#FFFDF5] border border-[#DCC78B] flex items-center justify-center text-[#8A6200]">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-2xl hover:bg-[#FFFDF5] text-slate-400 hover:text-slate-600 transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 border-b border-[#E7D9B5] bg-[#FFFDF5]/60 text-[11px] font-bold text-center">
          {[
            ...
            <div
              ...
              className={`py-2.5 border-b-2 cursor-pointer transition ${
                currentStep === s.num
                  ? 'border-[#A87A00] text-[#0F172A] bg-white'
                  : currentStep > s.num
            ...
            <div className="space-y-4 animate-fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-white border border-[#E7D9B5] rounded-2xl text-[#6B4D00] font-mono text-[11px] focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 bg-white border border-[#E7D9B5] rounded-2xl text-[#0F172A] focus:outline-none focus:border-[#A87A00]"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2 bg-white border border-[#E7D9B5] rounded-2xl text-[#0F172A] text-xs focus:outline-none focus:border-[#A87A00]"
                  />
                  <div className="w-10 h-10 rounded-xl overflow-hidden border border-[#E7D9B5] shrink-0 bg-white">
                    <img src={thumbnailUrl} alt="Thumbnail preview" className="w-full h-full object-cover" />
                  </div>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-[#E7D9B5]">
                <div
                  onClick={() => setTierAccess('free')}
                  className={`p-3 rounded-2xl border cursor-pointer transition ${
                    tierAccess === 'free'
                      ? 'border-[#A87A00] bg-white'
                      : 'border-[#E7D9B5] bg-white'
                  }`}
                >
                ...
                >
                  <span className="font-bold text-xs text-[#6B4D00] block">🔵 Pro Exclusive</span>
                ...
                >
                  <span className="font-bold text-xs text-[#8A6200] block">🟣 Paid Marketplace</span>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-[#E7D9B5]">
                    <input
                      type="color"
                      value={primaryAccent}
                ...
                {Object.entries(allowedBlocks).map(([blockKey, isEnabled]) => (
                  <div
                    ...
                    className={`p-3 rounded-2xl border flex items-center justify-between cursor-pointer transition ${
                      isEnabled
                        ? 'border-[#A87A00] bg-white'
                        : 'border-[#E7D9B5] bg-white opacity-60'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <Layers className="w-4 h-4 text-[#8A6200]" />
                    ...
                    <span
                      className={`w-5 h-5 rounded-lg flex items-center justify-center text-xs font-bold ${
                        isEnabled ? 'bg-[#8A6200] text-white' : 'bg-stone-200 text-slate-500'
                      }`}
                    >
          ...
          {currentStep === 4 && (
            <div className="space-y-4 animate-fade-in">
              <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <img
                      src={thumbnailUrl}
                      alt={name}
                      className="w-12 h-12 rounded-xl object-cover border border-[#E7D9B5]"
                    />
                    <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-[#E7D9B5] text-[11px]">
                  <div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#E7D9B5] bg-white">
          {currentStep > 1 ? (
            <button
              onClick={() => setCurrentStep((prev) => prev - 1)}
              className="flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-white hover:bg-[#FFF3CE] text-[#6B4D00] border border-[#E7D9B5] font-bold text-xs transition"
            >
          ...
          {currentStep < 4 ? (
            <button
              onClick={() => setCurrentStep((prev) => prev + 1)}
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-xs transition"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-6 py-2.5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-xs transition transform active:scale-95"
            >
