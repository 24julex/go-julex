...
export const CustomerDetailDrawer = ({ customer, isOpen, onClose }) => {
  ...
  const getChannelBadge = (channel = 'web') => {
    switch (channel.toLowerCase()) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#FFFDF5] text-[#6B4D00] border border-[#DCC78B]">
            <Globe className="w-3 h-3 text-[#8A6200]" /> Web
          </span>
        );
    }
  };
  ...
  const getFulfillmentBadge = (status = 'Delivered') => {
    ...
    if (s === 'delivered') {
      ...
      return (
        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#FFFDF5] text-[#6B4D00] border border-[#DCC78B]">
          Shipped
        </span>
      );
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end text-[#0F172A]">
      ...
      {/* Slide-Over Drawer */}
      <div className="relative w-full max-w-xl bg-white border-l border-[#E7D9B5] h-full shadow-2xl flex flex-col z-10 animate-slide-left">
        {/* Drawer Header */}
        <div className="p-6 border-b border-[#E7D9B5] bg-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <img
              ...
              alt={customer.name}
              className="w-12 h-12 rounded-2xl object-cover border border-[#E7D9B5] bg-white"
            />
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] space-y-1 shadow-xs">
              ...
              <div className="p-4 rounded-3xl bg-white border border-[#E7D9B5] space-y-1 shadow-xs">
                ...
                <p className="text-lg font-black font-mono text-[#8A6200]">
          ...
          <div className="p-5 rounded-3xl bg-white border border-[#E7D9B5] space-y-3 shadow-xs">
            <span className="font-bold text-[#0F172A] flex items-center gap-2 font-serif">
              <User className="w-4 h-4 text-[#8A6200]" /> Contact & Delivery Information
            </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
              <div className="space-y-1.5">
                <p className="text-[#374151] flex items-center gap-2">
                  ...
                  <a
                    ...
                    className="text-[#0F172A] hover:text-[#8A6200] hover:underline truncate"
                  >
                    {customer.email}
                  </a>
                </p>
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-wider text-[#6B4D00] flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-[#8A6200]" /> Primary Address
                </p>
          ...
          <div className="p-5 rounded-3xl bg-white border border-[#E7D9B5] space-y-4 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#0F172A] flex items-center gap-2 font-serif">
                <ShoppingBag className="w-4 h-4 text-[#8A6200]" /> Order History Timeline ({pastOrders.length})
              </span>
            </div>

            {pastOrders.length === 0 ? (
              <div className="p-8 text-center rounded-2xl bg-white border border-[#E7D9B5] space-y-2">
              ...
              <div className="divide-y divide-[#E7D9B5]/60">
                {pastOrders.map((order, idx) => (
                  <div key={idx} className="py-3 flex items-center justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-[#8A6200]">{order.id}</span>
