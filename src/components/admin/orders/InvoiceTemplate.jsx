  ...
  useEffect(() => {
    ...
    const applyConfig = (cfg) => {
      ...
      setInvoiceConfig(prev => ({
        ...
        legalName: cfg.storeLegalName || tenantName || prev.legalName,
...
${invoiceHtml}
  ...
  if (!previewConfig && String(order.paymentStatus || '').toUpperCase() !== 'PAID') {
    return <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full text-slate-900 shadow-xl">
        <h2 className="font-bold text-lg">Tax invoice unavailable</h2>
        <p className="mt-2 text-sm text-slate-600">Payment for this order has not been recorded. Use the order details to manage fulfillment; issue a tax invoice only after payment and store legal details are confirmed.</p>
        <button onClick={onClose} className="mt-5 rounded-xl bg-[#A87A00] px-4 py-2 text-white font-bold">Close</button>
      </div>
    </div>;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex justify-center p-4 sm:p-6 print:p-0 print:bg-white print:static print:overflow-visible">
      ...
      <div className="relative w-full max-w-4xl bg-white text-slate-900 rounded-3xl shadow-2xl overflow-hidden print:shadow-none print:rounded-none print:max-w-none flex flex-col my-auto">
        ...
        <div className="no-print flex items-center justify-between px-6 py-4 bg-white border-b border-[#E7D9B5]">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-[#D4A017] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-xs transition cursor-pointer"
            >
              <Printer className="w-4 h-4" /> Print / Save as PDF
            </button>
            <button
              onClick={onClose}
              className="p-2 rounded-2xl hover:bg-[#FFFDF5] text-slate-400 hover:text-slate-700 transition cursor-pointer"
              title="Close Preview"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        >
          ...
          {headerStyle === 'banner_strip' ? (
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-6 border-b border-slate-200 pb-6">
              ...
              <div className="text-left sm:text-right space-y-1 w-full sm:w-auto">
                ...
                <p className="text-xs text-slate-600">
                  <span className="font-bold text-slate-800">Payment:</span> {order.paymentMethod || 'Not recorded'}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-8 pt-2">
            ...
            <div className="space-y-3 max-w-sm text-xs">
              <div className="p-3.5 rounded-2xl bg-[#FFFDF5]/40 border border-[#E7D9B5] space-y-1">
                <span className="font-bold text-slate-900 block flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4 text-emerald-600" />
                  Payment: <span className="text-emerald-700 uppercase font-black">{order.paymentStatus}</span>
                </span>
                <p className="text-slate-600">
                  Method: <span className="font-semibold text-slate-800">{order.paymentMethod || 'Not recorded'}</span>
                </p>
