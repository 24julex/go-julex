...
export const OrderDetailModal = ({ order, isOpen, onClose, onOpenInvoice }) => {
  const { updateFulfillmentStatus, sendInvoiceEmail, recordCodCollection } = useMerchantAdmin();
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end text-[#0F172A]">
      ...
      {/* Slide-Over Drawer */}
      <div className="relative w-full max-w-2xl bg-white border-l border-[#E7D9B5] h-full shadow-2xl flex flex-col z-10 animate-slide-left">
        {/* Drawer Header */}
        <div className="p-6 border-b border-[#E7D9B5] flex items-center justify-between bg-white shrink-0">
          <div>
            ...
            <p className="text-xs text-[#374151] mt-1">
              ...
              <span className="font-semibold text-[#8A6200] uppercase">{order.channel}</span>
            </p>
          </div>

          <button
            onClick={onClose}
            className="p-2 rounded-2xl bg-white hover:bg-[#FFF3CE] text-slate-400 hover:text-slate-600 transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <button
              ...
              disabled
              title="Invoice email delivery is not configured"
              className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-2xl bg-[#FFFDF5] hover:bg-[#F8E8BD] border border-[#DCC78B] text-[#6B4D00] font-bold transition disabled:opacity-50"
            >
              {isSendingEmail ? (
                <>
                  <span className="w-3.5 h-3.5 border-2 border-[#A87A00] border-t-transparent rounded-full animate-spin" />
                  Sending...
                </>
              ) : (
                <>
                  <Send className="w-4 h-4 text-[#8A6200]" /> Email unavailable
                </>
              )}
            </button>
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-2xl bg-white hover:bg-[#FFF3CE] border border-[#E7D9B5] text-[#6B4D00] font-bold transition"
            >
              <Printer className="w-4 h-4 text-[#8A6200]" /> View Tax Invoice
            </button>

            {['UNFULFILLED', 'PROCESSING'].includes(String(order.fulfillmentStatus).toUpperCase()) ? (
              <button
                onClick={() => setIsFulfilling(!isFulfilling)}
                className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold transition shadow-xs"
              >
                <Truck className="w-4 h-4" /> Mark as Shipped
              </button>
            ) : String(order.fulfillmentStatus).toUpperCase() === 'SHIPPED' ? (
              <button
            ...
            ) : <span className="self-center text-center text-slate-500 font-semibold">{order.fulfillmentStatus}</span>}
          </div>

          {String(order.paymentMethod).toLowerCase() === 'cash on delivery' && String(order.fulfillmentStatus).toUpperCase() === 'DELIVERED' && String(order.paymentStatus).toUpperCase() === 'PENDING' && (
            <button onClick={async () => { if (await recordCodCollection(order.id)) onClose(); }} className="w-full rounded-2xl bg-[#A87A00] px-4 py-3 text-white font-bold">
          ...
          {isFulfilling && (
            <div className="p-4 rounded-3xl bg-white border border-[#D4A017] space-y-3 animate-fade-in shadow-xs">
              ...
              <button
                ...
                className="w-full py-2 bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold rounded-2xl shadow-xs transition"
              >
                Confirm Dispatch
              </button>
            </div>
          ...
          <div className="p-5 rounded-3xl bg-white border border-[#E7D9B5] space-y-3 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#0F172A] flex items-center gap-2 font-serif">
                <User className="w-4 h-4 text-[#8A6200]" /> Customer Information
              </span>
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#FFFDF5] text-[#6B4D00] border border-[#DCC78B] flex items-center gap-1">
                <Tag className="w-3 h-3 text-[#8A6200]" /> {order.customerSegment || 'Registered Customer'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
              ...
              <div className="space-y-1">
                <p className="text-[11px] font-bold uppercase tracking-wider text-[#6B4D00] flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-[#8A6200]" /> Shipping Destination
                </p>
          ...
          <div className="p-5 rounded-3xl bg-white border border-[#E7D9B5] space-y-3 shadow-xs">
            <span className="font-bold text-[#0F172A] flex items-center gap-2 font-serif">
              <Package className="w-4 h-4 text-[#8A6200]" /> Order Items ({order.items.length})
            </span>

            <div className="divide-y divide-[#E7D9B5]/60">
                ...
                <div key={idx} className="py-3 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <img
                      ...
                      className="w-12 h-12 rounded-2xl object-cover border border-[#E7D9B5] bg-slate-800"
                    />
                    <div>
          ...
          <div className="p-5 rounded-3xl bg-white border border-[#E7D9B5] space-y-3 shadow-xs">
            <span className="font-bold text-[#0F172A] flex items-center gap-2 font-serif">
              <CreditCard className="w-4 h-4 text-[#8A6200]" /> Payment & Invoice Breakdown
            </span>
            ...
            <div className="space-y-2 pt-1">
              ...
              <div className="pt-2 border-t border-[#E7D9B5] flex justify-between items-baseline">
                <span className="text-sm font-bold text-[#0F172A] uppercase tracking-wider">
                  (=) Total Order Amount:
                </span>
                <span className="text-lg font-black font-mono text-[#8A6200]">
                  ₹{(order.totalAmountINR ?? order.totalAmount ?? 0).toLocaleString('en-IN')}
                </span>
              </div>

              <div className="pt-2 flex items-center justify-between text-[11px] text-[#374151] bg-white p-2.5 rounded-2xl border border-[#E7D9B5] mt-2">
