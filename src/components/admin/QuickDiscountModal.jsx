...
export const QuickDiscountModal = ({ product, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0F172A]/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-lg rounded-3xl border border-[#E7D9B5] bg-white p-6 space-y-6 shadow-2xl animate-fade-in text-[#0F172A]">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-[#E7D9B5]">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl bg-[#FFFDF5] border border-[#DCC78B] flex items-center justify-center">
              <Percent className="w-5 h-5 text-[#8A6200]" />
            </div>
            <div>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-[#FFF3CE]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <span className="text-[#8A6200] font-bold absolute left-3.5 top-1/2 -translate-y-1/2">₹</span>
                <input
                  type="number"
                  min="0"
                  step="1"
                  ...
                  className="w-full pl-8 pr-4 py-2.5 text-sm bg-white border border-[#E7D9B5] rounded-2xl text-[#0F172A] font-mono focus:outline-none focus:border-[#A87A00]"
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-xs font-bold text-[#8A6200]">{discountPercent}% Off</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                className="w-full accent-[#A87A00] cursor-pointer"
              />
                ...
                {[0, 10, 15, 20, 25, 30, 50].map((pct) => (
                  <button
                    ...
                    className={`flex-1 py-1 rounded-xl text-[11px] font-bold border transition ${
                      discountPercent === pct
                        ? 'bg-[#8A6200] text-white border-[#A87A00] shadow-xs'
                        : 'bg-white border-[#E7D9B5] text-[#6B4D00] hover:bg-[#FFF3CE]'
                    }`}
                  >
                    {pct === 0 ? 'None' : `${pct}%`}
                  </button>
          ...
          <div className="p-4 rounded-2xl bg-white border border-[#E7D9B5] space-y-2 text-xs">
            <span className="text-[10px] font-bold text-[#6B4D00] uppercase tracking-wider block font-serif">
            ...
            <div className="flex justify-between text-[#0F172A] font-bold pt-2 border-t border-[#E7D9B5] text-sm">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              className="px-6 py-2.5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs flex items-center gap-2 transition shadow-xs"
            >
