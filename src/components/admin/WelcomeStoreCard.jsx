...
const PhoneField = ({ label, value, onChange }) => {
  ...
  return (
    <div>
      ...
      <div className="flex">
        <span className="px-3 py-2 border border-[#E7D9B5] border-r-0 rounded-l-xl text-sm font-mono font-bold text-slate-300 bg-white/5">+91</span>
        <input
          type="tel"
          value={digits}
          ...
          maxLength={10}
          className="w-full px-3 py-2 border border-[#E7D9B5] border-l-0 rounded-r-xl text-sm bg-white focus:outline-none focus:border-[#8A6200] font-mono"
        />
      </div>
    </div>
  );
...
export const WelcomeStoreCard = ({ storeData, onDone }) => {
  ...
  const inputCls = 'w-full px-3 py-2 border border-[#E7D9B5] rounded-xl text-sm bg-white focus:outline-none focus:border-[#8A6200]';
