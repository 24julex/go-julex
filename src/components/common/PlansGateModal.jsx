...
export const PlansGateModal = ({ open, onClose, contextTitle, contextMessage, storeStatusApi, onPlanSelected }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-3xl max-h-[88vh] overflow-y-auto bg-white rounded-3xl shadow-2xl border border-[#E7D9B5] p-7 space-y-5 text-[#0F172A]">
        ...
        {status && !status.canPublish && (
          <div className="text-[11px] font-bold px-3 py-2 rounded-xl bg-[#FFFDF7] border border-[#E7D9B5] text-[#6B4D00]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {!loading && plans.map((plan) => (
              <button
                ...
                disabled={loading}
                className={'text-left p-4 rounded-2xl border-2 transition cursor-pointer space-y-2 ' + (plan.isPopular ? 'border-[#D4A017] bg-[#FFFDF5]' : 'border-[#E7D9B5] bg-white hover:border-[#D4A017]')}
              >
                <div className="flex items-center justify-between">
                  <span className="font-serif font-black text-sm">{plan.name}</span>
                  {plan.badge && <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#FFFDF5] text-[#6B4D00]">{plan.badge}</span>}
                </div>
                <p className="font-mono font-black text-lg text-[#8A6200]">
        ...
        {phase === 'payment' && selected && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={acknowledgePayment} disabled={loading} className="px-4 py-2.5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs transition cursor-pointer">
                {loading ? 'Saving…' : 'Continue Editing'}
              </button>
              <button onClick={() => setPhase('plans')} className="px-4 py-2.5 rounded-2xl bg-white hover:bg-[#FFF3CE] border border-[#E7D9B5] text-[#6B4D00] font-bold text-xs transition cursor-pointer">
                Back to Plans
              </button>
            </div>
          </div>
        ...
        <div className="flex justify-between items-center pt-2 border-t border-[#E7D9B5]">
          <p className="text-[10px] text-stone-500">Plans are read live from the Go Julex database.</p>
          <button onClick={onClose} className="text-xs font-bold text-[#6B4D00] hover:underline cursor-pointer">Continue building without paying</button>
        </div>
      </div>
    </div>
  );
