...
export const CartProvider = ({ children }) => {
  ...
  const fetchCoupons = useCallback(async () => {
    try {
      const subdomain = cartItems[0]?.storeSubdomain || cartItems[0]?.tenantId;
      if (!subdomain) { setAvailableCoupons([]); return; }
      const res = await api.coupons.getActive(subdomain);
  ...
  // Apply only coupons returned by the backend; checkout revalidates them.
