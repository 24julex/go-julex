      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-center gap-2 text-[11px] font-bold">
          {['Store Identity', 'Domain', 'Plan'].map((label, i) => (
            <div key={label} className={'px-3 py-1.5 rounded-full border ' + (step === i + 1 ? 'bg-[#8A6200] text-white border-[#8A6200]' : 'bg-white text-[#6B4D00] border-[#E7D9B5]')}>{i + 1}. {label}</div>
          ))}
        </div>

        <div className="bg-white rounded-3xl border border-[#E7D9B5] p-6 space-y-4 shadow-sm">
          {step === 1 && (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold block mb-1">Store Name *</label>
                <input value={storeName} onChange={(e) => setStoreName(e.target.value)} placeholder="e.g. Bee's Sweets & Snacks" className="w-full px-3.5 py-2.5 border border-[#E7D9B5] rounded-2xl font-bold" />
              </div>
              <div>
                <label className="text-xs font-bold block mb-1">Category</label>
                <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-3.5 py-2.5 border border-[#E7D9B5] rounded-2xl bg-white">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="+91 98765 43210" className="w-full px-3 py-2 border border-[#E7D9B5] rounded-2xl" />
                </div>
                <div>
                  ...
                  <input value={instagram} onChange={(e) => setInstagram(e.target.value)} placeholder="@yourstore" className="w-full px-3 py-2 border border-[#E7D9B5] rounded-2xl" />
                </div>
              </div>
              <div>
                <label className="text-xs font-bold block mb-1">Owner Phone</label>
                <input value={ownerPhone} onChange={(e) => setOwnerPhone(e.target.value)} placeholder="+91 ..." className="w-full px-3 py-2 border border-[#E7D9B5] rounded-2xl" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-bold block">Store Logo</label>
                  {logoUrl ? <img src={logoUrl} alt="logo" className="h-16 w-16 rounded-2xl object-cover border border-[#E7D9B5]" /> : null}
                  <div className="flex gap-2">
                    <label className="px-3 py-1.5 rounded-xl bg-[#FFFDF5] border border-[#DCC78B] text-[#6B4D00] text-[11px] font-bold cursor-pointer">
                      {logoUrl ? 'Replace' : 'Upload'}
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) readFile(f, setLogoUrl); }} />
                    </label>
                    {logoUrl && <button onClick={() => setLogoUrl('')} className="text-[11px] font-bold text-stone-500 hover:text-[#6B4D00] cursor-pointer">Remove</button>}
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold block">Store Profile Picture</label>
                  {profileUrl ? <img src={profileUrl} alt="profile" className="h-16 w-16 rounded-full object-cover border border-[#E7D9B5]" /> : null}
                  <div className="flex gap-2">
                    <label className="px-3 py-1.5 rounded-xl bg-[#FFFDF5] border border-[#DCC78B] text-[#6B4D00] text-[11px] font-bold cursor-pointer">
                    ...
                    {profileUrl && <button onClick={() => setProfileUrl('')} className="text-[11px] font-bold text-stone-500 hover:text-[#6B4D00] cursor-pointer">Remove</button>}
                  </div>
                </div>
              </div>
            </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                ...
                <div className="flex items-center border border-[#E7D9B5] rounded-2xl px-3 py-2 bg-white">
                  <span className="text-stone-400 font-mono text-[11px]">https://</span>
                  <input value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9]/g, ''))} className="flex-1 bg-transparent px-1 font-mono font-bold text-[#8A6200]" />
                  <span className="text-stone-400 font-mono text-[11px]">.go.julex.shop</span>
                </div>
                {slugInfo && (
                  <p className={'text-[10.5px] mt-1 font-semibold ' + (slugInfo.available ? 'text-emerald-600' : 'text-[#8A6200]')}>{slugInfo.message}</p>
                )}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <button onClick={() => setPlansOpen(true)} className="px-5 py-3 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs transition cursor-pointer">View Go Julex Plans</button>
          ...
          {error && <p className="text-xs font-bold text-[#8A6200]">{error}</p>}

          <div className="flex justify-between pt-2 border-t border-[#E7D9B5]">
            <button onClick={() => (step > 1 ? setStep(step - 1) : navigate('/admin'))} className="px-4 py-2 rounded-2xl bg-white border border-[#E7D9B5] text-[#6B4D00] text-xs font-bold cursor-pointer">Back</button>
            {step < 3 ? (
              <button
                onClick={async () => { const ok = step === 1 ? await saveIdentity() : true; if (ok) setStep(step + 1); }}
                disabled={saving}
                className="px-5 py-2 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-xs font-bold disabled:opacity-60 cursor-pointer"
              >{saving ? 'Saving…' : 'Continue'}</button>
            ) : (
              <button onClick={finish} className="px-5 py-2 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-xs font-bold cursor-pointer">Go to Dashboard</button>
            )}
          </div>
        </div>
      </div>
