...
export const AdminSettings = () => {
  ...
  const inputCls = 'w-full px-3.5 py-2.5 bg-white border border-[#E7D9B5] rounded-xl text-sm text-[#0F172A] focus:outline-none focus:border-[#8A6200]';
  ...
  return (
    <div className="space-y-6 pb-16 text-[#0F172A]">
      {/* Header */}
      <div className="border-b border-[#E7D9B5] pb-4">
        <h1 className="text-xl sm:text-2xl font-bold font-serif flex items-center gap-2.5">
          <Settings className="w-6 h-6 text-[#8A6200]" /> Settings
        </h1>
        ...
        {[
          ...
          <button
            ...
            className={'px-4 py-2 rounded-2xl font-bold text-xs transition flex items-center gap-1.5 cursor-pointer ' +
              (activeTab === tab.id ? 'bg-[#8A6200] text-white' : 'bg-white border border-[#E7D9B5] text-[#6B4D00]')}
          >
      ...
      {activeTab === 'general' && (
        <form onSubmit={handleSaveProfile} className="p-6 rounded-3xl bg-white border border-[#E7D9B5] space-y-5 shadow-sm">
          <h3 className="font-bold text-sm flex items-center gap-2">
            <Building className="w-4 h-4 text-[#8A6200]" /> Store Identity
          </h3>
          ...
          {/* Branding */}
          <div className="pt-4 border-t border-[#E7D9B5]">
            <h3 className="font-bold text-sm flex items-center gap-2 mb-3">
              <Upload className="w-4 h-4 text-[#8A6200]" /> Store Branding
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold block mb-1">Store Logo</label>
                {storeProfile.logoUrl && <img src={storeProfile.logoUrl} alt="logo" className="h-16 w-16 rounded-2xl object-cover mb-2 border border-[#E7D9B5]" />}
                <div className="flex gap-2">
                  <label className="px-3 py-1.5 rounded-xl bg-[#FFFDF5] border border-[#DCC78B] text-[#6B4D00] text-xs font-bold cursor-pointer">
                  ...
                  {storeProfile.logoUrl && <button type="button" onClick={() => setStoreProfile({...storeProfile, logoUrl: ''})} className="text-xs text-stone-500 hover:text-[#6B4D00]">Remove</button>}
                </div>
              </div>
              <div>
                <label className="text-xs font-bold block mb-1">Profile Picture</label>
                {storeProfile.profileImageUrl && <img src={storeProfile.profileImageUrl} alt="profile" className="h-16 w-16 rounded-full object-cover mb-2 border border-[#E7D9B5]" />}
                <div className="flex gap-2">
                  <label className="px-3 py-1.5 rounded-xl bg-[#FFFDF5] border border-[#DCC78B] text-[#6B4D00] text-xs font-bold cursor-pointer">
                  ...
                  {storeProfile.profileImageUrl && <button type="button" onClick={() => setStoreProfile({...storeProfile, profileImageUrl: ''})} className="text-xs text-stone-500 hover:text-[#6B4D00]">Remove</button>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-[#E7D9B5]">
            <h3 className="font-bold text-sm flex items-center gap-2 mb-2">
              <Mail className="w-4 h-4 text-[#8A6200]" /> Store Domain
            </h3>
            <div className="p-3 rounded-xl bg-[#FFF9F6] border border-[#E7D9B5] text-xs font-mono">
          ...
          <button
            type="submit"
            disabled={saving}
            className="px-6 py-2.5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs transition disabled:opacity-60 cursor-pointer flex items-center gap-2"
          >
      ...
      {activeTab === 'plan' && (
        <div className="p-6 rounded-3xl bg-white border border-[#E7D9B5] space-y-4 shadow-sm">
          <h3 className="font-bold text-sm flex items-center gap-2">
            <CreditCard className="w-4 h-4 text-[#8A6200]" /> Plan & Billing
          </h3>
          <PlanInfoCard />
        </div>
      )}
      ...
      {activeTab === 'security' && (
        <div className="p-6 rounded-3xl bg-white border border-[#E7D9B5] space-y-4 shadow-sm">
          <h3 className="font-bold text-sm flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-[#8A6200]" /> Account Security
          </h3>
          <div className="p-3 rounded-xl bg-[#FFF9F6] border border-[#E7D9B5] text-xs space-y-1">
          ...
          <div className="p-4 rounded-2xl border border-[#E7D9B5] space-y-3" style={{ backgroundColor: currentUser?.twoFactorEnabled ? '#F0FDF4' : '#FFF9F6' }}>
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: 'rgba(212,160,23,0.12)', border: '1px solid rgba(212,160,23,0.3)' }}>
                <KeyRound className="w-4 h-4 text-[#8A6200]" />
              </div>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-xs font-bold cursor-pointer"
          >
            Log Out
          </button>
        </div>
