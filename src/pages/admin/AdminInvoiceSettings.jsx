...
export const AdminInvoiceSettings = () => {
  ...
  const selectedTemplate = availableTemplates.find(t => t.id === selectedTemplateId) || availableTemplates[0] || { name: 'Loading layouts', defaultLayout: {} };
    ...
    api.merchant.getInvoiceConfig(currentStore?.id)
      .then(res => {
        if (res?.success && res.data?.config) {
          ...
          if (cfg.templateId) { setSelectedTemplateId(cfg.templateId); setSavedTemplateId(cfg.templateId); }
          setLegalName(cfg.storeLegalName || '');
          setTradeName(cfg.storeTradeName || '');
          setGstin(cfg.storeGstin || '');
          setAddress(cfg.storeAddress || '');
          setPhone(cfg.storePhone || '');
          setEmail(cfg.storeEmail || '');
          setSignatureUrl(cfg.authorizedSignatoryUrl || '');
          if (cfg.customStyles) {
            ...
            setTerms(cfg.customStyles.terms || '');
          }
        }
        if (res?.data?.availableTemplates) {
  ...
  const handleSelectTemplate = (tpl) => {
    ...
    showToast(`"${tpl.name}" selected. Save invoice setup to apply it.`, 'info');
  };
  ...
  const handleSignatureUpload = (e) => {
    ...
    if (file) {
      if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type) || file.size > 2 * 1024 * 1024) {
        showToast('Choose a PNG, JPG, or WebP image under 2 MB.', 'error');
        return;
      }
  ...
  const handleSaveConfig = async () => {
    if (!legalName.trim() || !address.trim()) {
      showToast('Enter the store legal name and address before saving.', 'error');
      return;
    }
    const payload = {
    ...
    try {
      ...
      if (res?.success) {
        ...
        showToast('Invoice settings saved to the database.', 'success');
      } else {
        showToast(res?.message || 'Could not save invoice settings.', 'error');
      }
    } catch (err) {
      showToast('Could not save invoice settings.', 'error');
    }
  };

  const handlePrint = () => {
    showToast('This is a sample layout. Open a paid order to print its invoice.', 'info');
  };
        ...
        return (
          <div className="w-48 bg-white border border-stone-300 rounded-lg p-2.5 shadow-sm font-mono text-[8px] space-y-1.5 transform hover:scale-105 transition text-slate-900">
            ...
            <div className="space-y-1 text-stone-600">
              ...
              <div className="flex justify-between"><span>Illustrative tax</span><span>₹555</span></div>
            </div>
    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4" style={{ borderColor: 'var(--border-subtle)' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs mt-1" style={{ color: 'var(--text-secondary)' }}>
                Save your store identity and choose a layout. Confirm tax details before issuing real invoices.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            <Printer className="w-4 h-4" style={{ color: 'var(--accent)' }} /> Sample layout only
          </button>
          <button
      ...
      <div className="flex items-center gap-3 border-b pb-3" style={{ borderColor: 'var(--border-subtle)' }}>
        ...
        >
          <Sliders className="w-3.5 h-3.5" /> Invoice Settings & Sample Layout
        </button>
        ...
        <div className="ml-auto hidden md:flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
          ...
          <strong className="px-2.5 py-0.5 rounded-full border text-xs" style={{ backgroundColor: 'rgba(212,160,23,0.12)', color: 'var(--accent)', borderColor: 'rgba(212,160,23,0.25)' }}>
            {availableTemplates.find((t) => t.id === savedTemplateId)?.name || 'Not saved'}
          </strong>
        </div>
      </div>
      ...
      {activeTab === 'gallery' && (
        <div className="space-y-6">
          <div className="p-4 rounded-2xl border text-xs flex items-center justify-between" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-card)', color: 'var(--text-secondary)' }}>
            <span>
              Select a layout and save your settings. The layout does not calculate or certify GST.
            </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 border-t flex items-center justify-between" style={{ borderColor: 'var(--border-subtle)' }}>
                    ...
                    >
                      {isSelected ? <><Check className="w-4 h-4" /> Selected Layout</> : 'Select Layout'}
                    </button>
                    <button
