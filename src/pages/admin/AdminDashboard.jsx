...
export const AdminDashboard = () => {
  ...
  const cleanSubdomain = (currentStore?.subdomain || '').toLowerCase().replace(/\.go\.julex\.shop$/, '').replace(/\.gojulex\.com$/, '');
  ...
  const displayWhatsApp = storeAuth?.whatsappNumber || currentStore?.whatsappNumber || null;
  const displayInstagram = storeAuth?.instagramHandle || currentStore?.instagramHandle || null;

  const recentOrders = orders.slice(0, 5);

  const checklistSteps = [
    { title: 'Store instance provisioned', completed: true },
    { title: 'Connect custom domain SSL', completed: false },
    { title: 'Add catalog products & variants', completed: products.length > 0 },
    { title: 'Connect online payment gateway', completed: false },
    { title: 'Enable WhatsApp order integration', completed: false }
  ];
    ...
    <div className="space-y-6">
            ...
            {storeAuth && !storeAuth.canPublish && (
              <div className="p-4 rounded-2xl border border-amber-300 bg-amber-50 space-y-2 mb-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setPlansOpen(true)} className="px-3 py-1.5 rounded-xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-[11px] font-bold transition cursor-pointer">View Plans</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-3xl border space-y-2 shadow-sm" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-card)' }}>
          ...
          <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Paid orders placed today</p>
        </div>
        ...
        <div className="p-5 rounded-3xl border space-y-2 shadow-sm" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-card)' }}>
          ...
          <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Orders recorded for this store</p>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              Domain: <span className="font-mono font-semibold" style={{ color: 'var(--text-primary)' }}>{(() => {
                ...
                return currentStore?.customDomain || 'Not set';
              })()}</span>
            </p>
            <div className="pt-2 border-t text-[11px] flex items-center justify-between" style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-muted)' }}>
              <span>Custom-domain TLS is not configured</span>
          ...
          >
            ...
            <div className="pt-2 border-t text-[11px] flex items-center justify-between" style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-muted)' }}>
              <span>Manual customer contact</span>
          ...
          >
            ...
            <div className="pt-2 border-t text-[11px] flex items-center justify-between" style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-muted)' }}>
              <span>Social shop sync is not configured</span>
        ...
        <div className="lg:col-span-2 p-6 rounded-3xl border space-y-4 shadow-sm" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-card)' }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>Latest orders recorded for this store</p>
            </div>
            <Link
              to="/admin/orders"
