      ...
      <div className="flex items-center justify-between border-b border-[#E7D9B5] pb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold font-serif flex items-center gap-2.5">
            <CreditCard className="w-6 h-6 text-[#8A6200]" /> Billing & Plans
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {!loading && plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={'p-5 rounded-3xl border-2 space-y-3 flex flex-col ' + (isCurrent ? 'border-[#D4A017] bg-[#FFFDF5]' : 'border-[#E7D9B5] bg-white')}
            >
              {plan.badge && (
                <span className="inline-block px-2.5 py-0.5 rounded-full text-[9px] font-bold bg-[#FFFDF5] text-[#6B4D00]">{plan.badge}</span>
              )}
              <h3 className="font-serif font-black text-sm">{plan.name}</h3>
              <p className="font-mono font-black text-xl text-[#8A6200]">
              ...
              <button
                ...
                className={'w-full py-2.5 rounded-xl text-xs font-bold transition cursor-pointer ' + (isCurrent ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-[#8A6200] hover:bg-[#6B4D00] text-white disabled:opacity-50')}
              >
      ...
      {selected && (
        <div className="p-5 rounded-2xl border border-[#F5C842] bg-[#FFFDF5] space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setSelected(null)} className="px-4 py-2 rounded-xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-xs font-bold transition cursor-pointer">Continue Building</button>
          </div>
        </div>
      )}

      {message && <p className="text-xs font-bold text-[#8A6200]">{message}</p>}
    </div>
  );
};
