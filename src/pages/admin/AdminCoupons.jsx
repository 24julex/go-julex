...
export const AdminCoupons = () => {
  ...
  return (
    <div className="space-y-8 text-[#0F172A] pb-16">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E7D9B5] pb-6">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <Tag className="w-5 h-5 text-[#8A6200]" />
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2.5 rounded-2xl border border-[#E7D9B5] hover:bg-[#FFF3CE] bg-white text-[#6B4D00] transition shadow-xs"
          ...
          <button
            ...
            className="py-2.5 px-5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-xs flex items-center gap-2 transition hover:scale-[1.02]"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-3xl border border-[#E7D9B5] bg-white shadow-xs space-y-1">
          <div className="flex items-center justify-between text-[#374151] mb-1">
            ...
            <Tag className="w-4 h-4 text-[#8A6200]" />
          </div>
        ...
        <div className="p-5 rounded-3xl border border-[#E7D9B5] bg-white shadow-xs space-y-1">
          <div className="flex items-center justify-between text-[#374151] mb-1">
            ...
            <Percent className="w-4 h-4 text-[#8A6200]" />
          </div>
          <p className="text-2xl font-serif font-bold text-[#8A6200]">{maxPercent}% OFF</p>
      ...
      {/* Search & Filter Bar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-3xl bg-white border border-[#E7D9B5] shadow-xs">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-white border border-[#E7D9B5] rounded-2xl text-[#0F172A] text-xs placeholder:text-slate-400 focus:outline-none focus:border-[#A87A00]"
          />
        </div>
        <p className="text-xs text-[#374151]">
      ...
      {/* Coupons Table */}
      {loading ? (
        <div className="p-12 text-center rounded-3xl border border-[#E7D9B5] bg-white flex flex-col items-center justify-center gap-3">
          <Loader2 className="w-8 h-8 text-[#8A6200] animate-spin" />
          <p className="text-xs text-[#374151] font-semibold tracking-wider uppercase">
            Loading Coupons Registry...
          </p>
        </div>
      ) : filteredCoupons.length === 0 ? (
        <div className="p-12 text-center rounded-3xl border border-[#E7D9B5] bg-white space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-[#FFFDF5] border border-[#DCC78B] flex items-center justify-center mx-auto text-[#8A6200]">
          ...
          <button
            onClick={handleCreate}
            className="py-2.5 px-5 rounded-2xl bg-[#8A6200] hover:bg-[#6B4D00] text-white font-bold text-xs shadow-xs"
          >
            Create First Coupon
          </button>
        </div>
      ) : (
        <div className="rounded-3xl border border-[#E7D9B5] bg-white overflow-hidden shadow-xs">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-[#E7D9B5] bg-[#FFE4E6]/60 text-[#6B4D00] font-semibold uppercase tracking-wider text-[11px]">
              ...
              <tbody className="divide-y divide-[#E7D9B5]/60 text-[#0F172A]">
                {filteredCoupons.map((c) => {
                  ...
                  return (
                    <tr
                      key={c.id || c.code}
                      className="hover:bg-[#FFF3CE]/40 transition duration-150 group"
                    >
                      {/* Code */}
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs font-bold text-[#8A6200] bg-[#FFFDF5] border border-[#DCC78B] px-2.5 py-1 rounded-xl">
                            {c.code}
                          </span>
                          <button
                            onClick={() => handleCopyCode(c.code)}
                            className="p-1 text-slate-400 hover:text-[#8A6200] transition"
                            title="Copy code"
                          >
                      ...
                      <td className="py-4 px-6">
                        {c.discountType === 'PERCENT' ? (
                          ...
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-[#FFFDF5] border border-[#DCC78B] text-[#6B4D00] font-bold">
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="p-2 rounded-xl bg-white border border-[#E7D9B5] hover:bg-[#FFF3CE] text-[#6B4D00] transition"
                            title="Edit coupon"
                          >
                            <Edit2 className="w-3.5 h-3.5 text-[#8A6200]" />
                          </button>
                          <button
                            onClick={() => handleDelete(c.id, c.code)}
                            className="p-2 rounded-xl bg-white border border-[#E7D9B5] hover:bg-rose-50 text-slate-400 hover:text-rose-600 transition"
                            title="Delete coupon"
                          >
