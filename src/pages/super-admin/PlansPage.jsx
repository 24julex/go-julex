...
const PlanEditModal = ({ plan, onSave, onClose, busy }) => {
  ...
  const inputCls = 'w-full px-3 py-2 border rounded-xl text-sm bg-white focus:outline-none focus:border-[#8A6200]';
    ...
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl border border-slate-200 overflow-hidden max-h-[85vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-2">
          ...
          <button onClick={handleSave} disabled={busy || (form._isNew && !form.id) || !form.name} className="px-5 py-2 rounded-xl bg-[#8A6200] hover:bg-[#6B4D00] text-white text-xs font-bold disabled:opacity-50 cursor-pointer">
