...
async function testMutations() {
  ...
  await check('POST /api/coupons/validate', async () => {
    const merchantLogin = await fetch(`${BASE}/auth/login`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'merchant@gojulex.com', password: 'admin123' })
    });
    const merchant = await merchantLogin.json();
    if (!merchant.token) throw new Error('Merchant login failed');
    const code = `TEST${Date.now()}`;
    const created = await fetch(`${BASE}/coupons`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${merchant.token}` },
      body: JSON.stringify({ code, description: 'Test discount', discountType: 'PERCENT', discountValue: 10 })
    });
    if (created.status !== 201) throw new Error('Could not create test coupon');
    const res = await fetch(`${BASE}/coupons/validate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code, cartTotal: 50000, subdomain: merchant.user.tenant.subdomain })
    });
    const json = await res.json();
