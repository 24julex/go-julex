...
if (process.env.NODE_ENV === 'production' && (JWT_SECRET.length < 32 || /change-me|gojulex_saas_multi_tenant_super_secret/i.test(JWT_SECRET))) {
  console.error('FATAL: Configure a unique JWT_SECRET of at least 32 characters before production startup.');
  process.exit(1);
}
...
export const requireMerchantAdmin = async (req, res, next) => {
  await requireAuth(req, res, () => {
    ...
    if (isMerchant && !req.user.tenantId) {
      return res.status(403).json({ success: false, message: 'No store is linked to this account.' });
    }
...
export const optionalAuth = (req, res, next) =>
  req.headers.authorization ? requireAuth(req, res, next) : next();

// A merchant's tenant comes only from the verified session. A super admin may
// work across tenants unless they explicitly entered an impersonation session.
export const tenantScope = (req) =>
  req.user?.role === 'SUPER_ADMIN' && !req.impersonatedTenantId
    ? {}
