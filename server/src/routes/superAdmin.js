 ...
 * Record a proposed platform broadcast. Delivery is not connected yet.
 */
router.post('/broadcast', requireSuperAdmin, async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message: `Broadcast "${title}" recorded for ${targets.length} store(s). Delivery is not configured yet.`,
