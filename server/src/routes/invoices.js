    ...
    if (!config) {
      ...
      config = await prisma.tenantInvoiceConfig.create({
        data: {
          ...
          storeGstin: '',
          storeLegalName: tenant?.name || 'Store name pending configuration',
          storeTradeName: tenant ? tenant.name : 'Store Name',
          storeAddress: tenant ? [tenant.city, tenant.state].filter(Boolean).join(', ') : '',
          storePhone: '',
          storeEmail: '',
      ...
      if (tenant) {
        ...
        config = await prisma.tenantInvoiceConfig.create({
          data: {
            ...
            storeLegalName: tenant.name,
            storeTradeName: tenant.name,
            storeAddress: [tenant.city, tenant.state].filter(Boolean).join(', '),
            storeEmail: '',
            customStylesJson: JSON.stringify({
              fontFamily: 'Inter',
              fontSize: 12,
...
router.get('/merchant/invoices/render/:orderId', requireMerchantAdmin, async (req, res) => {
  try {
    ...
    if (order.paymentStatus !== 'PAID') {
      return res.status(409).json({ success: false, message: 'A tax invoice can only be generated after payment is recorded.' });
    }
    ...
    if (!config?.storeLegalName || !config?.storeAddress) {
      return res.status(409).json({ success: false, message: 'Complete the store legal name and address before generating a tax invoice.' });
    }
    ...
    const invoicePayload = {
      ...
      store: {
        legalName: config.storeLegalName,
        tradeName: config?.storeTradeName || order.tenant?.name,
        gstin: config.storeGstin || '',
