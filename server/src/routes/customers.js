...
router.get('/', requireAdmin, async (req, res) => {
  try {
    ...
    const users = await prisma.user.findMany({
      where: { role: 'USER', ...(req.user.role === 'SUPER_ADMIN' ? {} : { orders: { some: { tenantId: req.tenantId } } }) },
      include: {
        orders: {
          where: req.user.role === 'SUPER_ADMIN' ? {} : { tenantId: req.tenantId },
...
router.get('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (!user || user.role !== 'USER' || (req.user.role !== 'SUPER_ADMIN' && user.orders.length === 0)) {
