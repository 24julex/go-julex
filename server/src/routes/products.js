...
router.post('/', requireAdmin, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'SUPER_ADMIN' && !req.tenantId) {
      return res.status(403).json({ success: false, message: 'No store linked to this account.' });
    }
    if (req.user.role !== 'SUPER_ADMIN' && tenantId && tenantId !== req.tenantId) {
      return res.status(403).json({ success: false, message: 'Not your store.' });
    }
    let productTenantId = req.user.role === 'SUPER_ADMIN' ? (tenantId || req.tenantId || null) : req.tenantId;
    if (productTenantId) {
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'SUPER_ADMIN' && tenantId !== undefined && tenantId !== req.tenantId) {
      return res.status(403).json({ success: false, message: 'Cannot move a product to another store.' });
    }
...
router.post('/:id/reviews', requireAuth, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'USER') return res.status(403).json({ success: false, message: 'Sign in as a customer to review a purchase.' });
    if (!String(comment || '').trim()) return res.status(400).json({ success: false, message: 'Write a review before submitting.' });
    const purchase = await prisma.order.findFirst({ where: { userId: req.user.id, paymentStatus: 'PAID', items: { some: { productId: req.params.id } } } });
    if (!purchase) return res.status(403).json({ success: false, message: 'Only customers with a paid purchase can review this product.' });
